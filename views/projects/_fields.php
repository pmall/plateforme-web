...
<fieldset>
  ...
  <ul class="list">
	...
	<? $error = ''; if($project->hasError($celfile)){ $error = ' class="error"'; } ?>
        ...
        <input id="chip_<?= $i ?>" name="project[chips][<?= $celfile ?>][num]" type="text" size="3" value="<?= h($project->getChipNum($celfile)) ?>" placeholder="Num" />
        <input name="project[chips][<?= $celfile ?>][condition]" type="text" value="<?= h($project->getChipCondition($celfile)) ?>" maxlength="20"  placeholder="Condition" />
      </div>
    </li>
    <? endforeach; ?>
  </ul>
